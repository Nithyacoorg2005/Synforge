 ...
 * Reusable card for statistical fidelity and utility metrics.
 */
...
const SynForgeDashboard = () => {
    ...
    const handleUpload = async () => {
        ...
        // Engineering Validation: Client-side warning for small datasets
        if (file.size < 5000) {
            alert("Warning: Dataset size is small. Privacy Risk (MIA) may be significantly higher due to insufficient statistical cover.");
        }
    ...
    // Polling logic for status updates from backend
    useEffect(() => {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 p-8 font-mono selection:bg-blue-500/30">
            <header className="border-b border-slate-800 pb-6 mb-10 flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 text-xs mt-1 uppercase tracking-widest">
                        Adversarial Privacy & Synthetic Data Engine
                    </p>
                </div>
                {status === 'completed' && metrics && (
                    <StatusBadge risk={metrics.privacy_risk} />
                )}
            </header>

            {/* STATE 1: CONFIGURATION & UPLOAD */}
            {status === 'idle' && (
                <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl">
                        ...
                        />
                        <div className="flex justify-between text-[9px] text-slate-600 mt-3 uppercase font-bold tracking-tighter">
                            <span>Strict Anonymization (High Noise)</span>
                            <span>High Fidelity Utility (Low Noise)</span>
                        </div>
                    </div>
                    ...
                    <div className="border-2 border-dashed border-slate-800 rounded-xl p-16 text-center bg-slate-900/20 hover:border-blue-500/50 transition-all group">
                        ...
                        <label htmlFor="fileInput" className="cursor-pointer flex flex-col items-center">
                            ...
                            <span className="text-slate-500 text-xs uppercase tracking-widest italic">CSV format required for audit</span>
                        </label>

                        {file && (
                            <div className="mt-8 pt-8 border-t border-slate-800/50 animate-in zoom-in-95">
                                <p className="mb-6 text-[10px] bg-slate-800 py-1.5 px-4 rounded-full text-slate-300 border border-slate-700 uppercase inline-block">
                                    {file.name}
                                </p>
                                <br />
                                <button onClick={handleUpload} className="bg-blue-600 hover:bg-blue-500 active:scale-95 transition-all px-12 py-4 rounded text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-900/20">
                                    Execute Synthesis
                                </button>
                            </div>
                        )}
            ...
            {status === 'processing' && (
                <div className="flex flex-col items-center justify-center py-32 space-y-6">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-slate-500 text-[10px] mt-2 uppercase tracking-widest animate-pulse">Injecting Gradient Noise // Auditing Leakage</p>
                    </div>
                </div>
            )}
                ...
                <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <MetricCard
                            ...
                            value={`${(metrics.fidelity * 100).toFixed(1)}%`}
                            sub="Distribution Accuracy"
                            color="text-emerald-400"
                        />

                        <MetricCard
                            title="Privacy Risk"
                            value={metrics.privacy_risk < 0.3 ? "LOW" : metrics.privacy_risk < 0.7 ? "MED" : "HIGH"}
                            sub={`MIA Attack: ${metrics.privacy_risk}`}
                            color={metrics.privacy_risk < 0.3 ? "text-blue-400" : "text-rose-400"}
                            border={metrics.privacy_risk > 0.7 ? "border-rose-900/50" : "border-slate-800"}
                        />
                        ...
                        <div className="bg-slate-900 p-6 rounded-lg border border-slate-800 flex flex-col gap-3 justify-center shadow-lg">
                            <a
                                ...
                                className="w-full bg-blue-600 hover:bg-blue-500 transition-all py-3 rounded flex items-center justify-center gap-2 font-black no-underline text-white uppercase tracking-widest text-[10px]"
                            >
                                <Download size={14} /> Synthetic CSV
                            </a>

                            <a
                                href={`http://localhost:8000/audit-report/${jobId}`}
                                className="w-full bg-transparent border border-slate-700 hover:border-slate-500 transition-all py-3 rounded flex items-center justify-center gap-2 font-black no-underline text-slate-300 uppercase tracking-widest text-[10px]"
                            >
